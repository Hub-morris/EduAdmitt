...
export function getAdminApplicationDetailQuery() {
  return `
    SELECT
      a.id,
      a.student_id,
...
router.get('/applications/:id', async (req, res) => {
  try {
    const result = await pool.query(getAdminApplicationDetailQuery(), [req.params.id]);
...
// Serve uploaded documents by id (stream file from uploads directory)
router.get('/documents/:id', async (req, res) => {
  try {
    const docRes = await pool.query('SELECT * FROM application_documents WHERE id = $1', [req.params.id]);
